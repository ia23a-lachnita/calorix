...
- [ ] **Slice G — deterministic catalog lookup and fail-closed barcode resolution:** distinguish an authoritative catalog miss from timeout/network/rate-limit/server failure; never silently translate provider failure into `null`. For a supplied/raw barcode, a provider failure must return a stable provider failure without invoking vision, and a genuine catalog miss must remain unconfirmed/Review rather than becoming a visual nutrition estimate. Freeze focused OFF-client, evaluator-adapter, and production-analyze tests before implementation; keep prompts, meal/label normalization, Flutter UI, scoring thresholds, Firebase, deployment, device work, and corpus truth out of scope. After GREEN/full/build/lint/review/commit/push, rerun the three-sample public barcode subset or exact full public gate and require zero barcode vision fallbacks/catastrophic estimates when a barcode is supplied.
