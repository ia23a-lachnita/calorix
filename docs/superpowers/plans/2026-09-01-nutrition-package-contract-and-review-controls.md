...
- [x] **Slice E — catalog multipack name evidence:** reproduce live OFF case `4056489686941` test-first; allow only strict reliable `NxU` evidence from `product.name`, preserve malformed/unsupported structured and raw-quantity precedence, fail closed when reliable name/raw multipacks disagree, and keep a structured single-unit conflict as outer-package totals plus `nutrition_basis_ambiguous` with no `consumedAmount`. Verify focused/full Functions, build, lint, and diff check; obtain post-review; commit/push; then rerun the exact 20-public/0-private live gate at the new SHA. Do not change the manifest truth or meal/label policy.

**Actual (2026-09-11):** pushed source `ae7a34b371f60551e6057adf89d61d0039313722` produced run `run-2026-09-11T06-03-43-886Z`: **20/20 parsed, 0 failures, 0 unsafe completions, 4 catastrophic results all routed to Review**, with median/P90 relative kcal error **35.01% / 95.94%**, mean macro relative error **51.13%**, and Review rate **85%**. The corrected multipack is exact and Review-only. Safety is green, but meal/label accuracy remains red; a separately approved diagnostic slice is required before three-sample calibration or Task 10.
...
- [x] **Slice F — privacy-safe calorie and macro error decomposition:** add optional strict numeric-only prediction diagnostics for raw kcal/protein/carbs/fat, detected count/positive total mass, declared tuple, observed pair, and complete references; never persist names or arbitrary strings. Preserve existing total kcal/macro/catastrophic/safety metrics. Add zero-aware diagnostic metrics for meal mass, complete four-nutrient meal per-100 density, and complete four-nutrient label per-100 extraction; ratio/relative are present only when truth is positive, while zero truth retains predicted/truth/absolute error. Require all-or-none/nonfinite invariants, OFF provenance without fabricated model evidence, cache round-trip compatibility, and a separate optional Markdown diagnostics table without changing the existing Cases table. TDD/focused/full/build/lint/review/commit/push precede a three-sample run over exactly 20 public / 0 private cases; inspect all 60 calorie and macro results plus variance before any accuracy fix or Task 10.
  ...
  **GREEN implementation reviewed (2026-09-11):** review-driven RED rounds exposed 8 then 5 missing edge behaviors; final focused **208/208**, full Functions **648 passed / 1 skipped**, build, lint, and diff-check pass. Independent review is approved with no findings, and Antigravity conversation `calorix-live-eval-calibration-20260911` returned `AGREEMENT_STATUS: agree`, `MUST_FIX: none`. The implementation is pushed at `bb414d1850fb9f91cc419b4a270138354abf5535`.

  **Three-sample measurement complete (2026-09-11):** ignored run `run-2026-09-11T20-22-21-450Z` used the pushed Slice F SHA, `calorix-public-v1` hash `2dc17d06752c2981862690953a7b134235bb6a20da4dc9b5fef5528f91f5bb56`, unchanged prompt hash `205b635a252e1f378023f5e1f3c670a6fba0ecfdfc8ce4f08f30efa24c544263`, `gemini-2.5-flash`, exactly 20 public / 0 private cases, and three uncached samples. All **60/60** rows were inspected: **60 parsed, 0 failures, 0 unsafe completions, 52 Review, 15 catastrophic calorie misses**. Aggregate median/P90 relative calorie error is **27.19% / 101.36%** and mean total macro relative error is **52.27%**. Every individual sample still has five catastrophic misses and 50.20–54.05% mean macro error; only 6/20 cases repeat an identical full nutrient vector across all samples. Meal rows are mass-dominated in 31/36, but independent per-100 density error remains severe for carbohydrates (**77.80% mean**) and fat (**37.86% mean**). Label per-100 density is much stronger where present—kcal **0.056%**, protein **8.37%**, carbohydrates **15.86%**, fat **0.887%** mean relative error—but ungrounded package quantities cause 7/12 catastrophic total results. Barcode catalog data is exact in 11/12 rows; one transient catalog miss silently degraded to a catastrophic vision fallback. Safety is green; calorie and macro accuracy are not.

- [x] **Slice G — deterministic catalog lookup and fail-closed barcode resolution:** distinguish an authoritative catalog miss from timeout/network/rate-limit/server failure; never silently translate provider failure into `null`. For a supplied/raw barcode, a provider failure must return a stable provider failure without invoking vision, and a genuine catalog miss must return a stable product-not-found error rather than fabricating a nutrition draft or invoking vision because no trustworthy nutrient vector exists to review. Freeze focused OFF-client, evaluator-adapter, and production-analyze tests before implementation; keep prompts, meal/label normalization, Flutter UI, scoring thresholds, Firebase, deployment, device work, and corpus truth out of scope. After GREEN/full/build/lint/review/commit/push, rerun the three-sample public barcode subset or exact full public gate and require zero barcode vision fallbacks/catastrophic estimates when a barcode is supplied.

  **Design review (2026-09-12):** mandatory read-only Antigravity continuation `calorix-live-eval-calibration-20260911`, strongest listed `gemini-3.8-flash`, independently confirmed the live root causes and returned exact `AGREEMENT_STATUS: agree`, `MUST_FIX: none`, `SHOULD_FIX: none`, `QUESTIONS: none`. It ranks Slice G first, source-labeled Task 10 amount selection second (or parallel with meal research), and meal mass plus complete macro-density improvement as a separate held-out-data calibration slice. Public-corpus prompt tuning and metric changes remain prohibited because 12 public meals are too narrow and already inspected.

  **RED frozen (2026-09-12):** only `off-client.test.ts`, evaluator `live-adapter.test.ts`, and `analyze-entry.test.ts` changed. Fresh host witness reports **7 intended failures / 85 passes (92 total)**: five OFF transport/timeout/429/500/503 failures are still swallowed, one supplied-barcode evaluator miss still reaches vision, and one production raw-barcode miss still loads vision and persists the wrong error. Exact 404 and parseable `product_not_found` remain `null`; typed errors require an exact privacy-safe name/code/message with no cause/raw diagnostic; fake-timer cleanup covers success, 404, provider HTTP failure, and abort; no-raw-barcode vision detection remains green. Test ESLint and diff-check pass. Independent re-review returned `SAFE_TO_FREEZE: yes`, no Critical/Important findings, `MUST_FIX: none`. GREEN production is next.

  **GREEN reviewed / live evidence next (2026-09-12):** the OFF client now exports a fixed privacy-safe provider error, keeps only exact HTTP 404 and exact 2xx `failure/product_not_found` as authoritative misses, maps transport/abort/non-404 HTTP errors and other recognized 2xx failure envelopes to provider failure, and clears its timer on every path. Supplied evaluator and raw production barcodes stop before image/vision on miss or failure; a genuine miss becomes `off_product_not_found`. A review-driven 2xx `rate_limited` regression failed **1/28** before the fix. Final fresh host verification is focused **93/93**, full Functions **656 passed / 1 intentional skip**, build, lint, and diff-check green with no unhandled rejection. Independent review returned `SPEC_COMPLIANCE: pass`, `APPROVED: yes`, no findings, `MUST_FIX: none`. Mandatory Antigravity continuation `calorix-live-eval-calibration-20260911`, `gemini-3.8-flash`, returned exact `AGREEMENT_STATUS: agree`, `MUST_FIX: none`, `SHOULD_FIX: none`, `QUESTIONS: none`; its wrapper claimed background build/lint activity despite the no-write prompt, recorded as response noise rather than host evidence, and git inspection found no unexpected mutation. Commit/push and the three-sample public barcode live gate remain.

  **Live barcode evidence complete (2026-09-12):** implementation commit `d9492b60d06296b54f51d951b0d5fb4ae8c89ed8` is pushed and remote-equal. After deterministic evaluation fixtures **334 passed / 1 skipped** and a fresh build, ignored subset run `run-2026-09-12T01-09-48-372Z` exercised exactly four public supplied-barcode cases × three uncached samples using the real OFF client and a poison vision adapter. All **12/12** outcomes were inspected and `visionCalls` was exactly **0**. Eleven catalog results were exact for kcal, protein, carbohydrates, and fat; the same flaky `off-7622210449283` third sample reproduced as one explicit `provider/provider_request_failed`, not a vision result. Summary: **11 parsed / 1 provider failure, 0 catastrophic, 0 unsafe, median/P90 calorie error 0, mean macro error 0** across parsed rows. This proves fail-closed routing, not provider availability; retry/backoff remains intentionally out of scope.

- [x] **Step 6: Record, commit, and push the evaluation checkpoint**
