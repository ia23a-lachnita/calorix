...
- [x] **Step 2: RED** — pinned Flutter 3.41.9 scan compile failed only because production still requires `ScanUploadGateway.enqueueAndUpload`; the isolated real queue suite passed 29 existing tests and failed the new concurrency test with `Expected: 1, Actual: 2`.
- [x] **Step 3: Implement queue ownership** — replace `UploadQueueService.enqueueAndUpload` with `enqueueScan(...)`, which owns entry-ID creation through `_entryIdFactory`, storage-path construction, durable copy, and KV persistence but never starts transport. Guard `drain` with per-entry in-flight state so an immediate scheduled drain and `UploadRetryCoordinator` cannot upload the same queue item concurrently; always clear the guard in `finally`.
- [x] **Step 4: Implement gateway and UI ordering** — replace the gateway's combined operation with exact responsibilities: `enqueue(...) -> Future<String>` delegates to `UploadQueueService.enqueueScan`; `scheduleDrain() -> void` starts `drainPending()` and consumes its Future errors so no unhandled zone error escapes while queue state remains authoritative. In `_processImage`, await only durable enqueue, show a Scan-screen-local captured-photo overlay for at most `MotionDurations.cardEntrance` (zero under reduced motion), navigate using only `/processing/:id`, then schedule drain exactly once. Do not pass local paths in route extras. Update every fake/call site and remove the combined gateway method so future UI code cannot regress to awaiting transport.
- [x] **Step 5: GREEN** — focused scan and queue tests pass. `fvm flutter analyze` → clean.
- [x] **Step 6: HANDOFF**
  ```bash
