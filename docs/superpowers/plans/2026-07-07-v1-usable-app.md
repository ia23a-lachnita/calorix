...
Date: 2026-07-07
Status: stages 1–5 executed and verified (2026-07-08); later stages partially implemented; release blockers documented in [the 2026-07-10 Fable session audit](../../audits/2026-07-10-fable-session-review.md)

Progress (2026-07-10): stages 1–5 are implemented and later UI/backend stages are partially implemented. The code-review evidence in [the Fable session audit](../../audits/2026-07-10-fable-session-review.md) identifies release blockers; it does not claim those blockers are fixed. The active blocker-remediation design and execution sequence are now [the production-correctness spec](../specs/2026-07-10-calorix-production-correctness.md) and [plan](2026-07-10-calorix-production-correctness.md). Fresh verification: `fvm flutter analyze` passed with no issues; `fvm flutter test` passed 76 tests; `cd functions && npm test` passed 32 tests in 5 files; `cd functions && npm run test:rules` passed 12 tests. Residual warnings: the reference-image source is moved, and cloud/deploy state still needs explicit reproducible evidence.
