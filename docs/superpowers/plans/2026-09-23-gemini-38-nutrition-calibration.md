...
**Selection contract:** Pin and verify these exact official public objects before parsing: `dish_ids/splits/rgb_train_ids.txt`, `metadata/dish_metadata_cafe1.csv`, and `metadata/dish_metadata_cafe2.csv` below `https://storage.googleapis.com/nutrition5k_dataset/nutrition5k_dataset/`. The split contains trimmed unique `dish_[0-9]+` IDs. Parse CSV as RFC 4180. The pinned official bytes have six fixed fields `dish_id,total_calories,total_mass,total_fat,total_carb,total_protein`, followed by one or more seven-field ingredient groups `(ingredient_id,name,grams,calories,fat,carb,protein)`; derive component count as `(columnCount - 6) / 7`, require an exact positive integer, validate finite numeric fields and every ingredient group, and fail if the pinned structure changes. Duplicate IDs, malformed or non-finite numbers, invalid group counts, empty ingredient IDs, non-positive ingredient grams, or negative ingredient nutrient values fail structurally; ingredient names are unused descriptive metadata and may be empty in the official upstream files. Candidate nutrition semantics apply only after intersecting merged metadata with the train split and removing the 12 frozen test IDs: candidates require positive total mass and calories plus non-negative total macros. Any train-split candidate that fails those total-nutrition semantics or is absent from merged metadata is excluded from selection and recorded in `calibration-source-lock.json` under `excludedDishes` with a stable typed reason; non-train rows never enter that list. `--verify` reconstructs the exclusions from the pinned bytes and fails if their exact canonical list differs from the committed lock. Eligible IDs otherwise have a valid official overhead `rgb.png`.
...
- [x] **Step 1: Write RED pure-selection tests.** Use tiny committed test fixtures to prove exact source paths, source-hash rejection before parsing, RFC 4180 quoting, six fixed fields plus seven-field ingredient-group derivation, changed-format/duplicate/malformed rejection, exact one/two/three-plus bins, all calorie boundaries and macro tie precedence, frozen-test disjointness, stable rank, development-first shared-ID selection, the exact listed slot schedule/margins, rejection of any slot-list drift, HTTP/media/signature/dimension validation through `inspectImage`, deterministic failed-image skip within one stratum, exhaustion failure without neighboring fallback, exact 24/16 partitioning, and identical output after input-row reordering.
- [x] **Step 2: Run RED.** Run `cd functions && npx vitest run test/nutrition-eval/calibration-corpus.test.ts`.
- [x] **Step 3: Implement the pure selector and two explicit opt-in modes.** Default tests consume in-memory fixtures only. First-time `--pin` fails if a lock or manifest already exists, fetches official public bytes, records exact source/image SHA-256 values plus stable skipped-image reasons, and writes through atomic replacement. Normal `--verify` requires the existing committed pins and fails on any upstream/source/image mismatch without rewriting or substituting. There is no trust-on-first-use behavior hidden inside verification.
- [x] **Step 4: Add a strict calibration-manifest schema.** Keep the generic version-1 manifest backward compatible, but parse calibration data through a strict schema that retains and validates `group: 'development' | 'validation'`, fixed stratum fields, rank, attribution, source-lock hash, and all case data. Canonicalize and hash the full strict object so group/split changes alter `datasetHash`; unknown keys fail rather than being stripped.
- [x] **Step 4a: Add RED coverage for official semantic exclusions.** Prove that structurally valid zero totals parse, non-train invalid totals are ignored for eligibility, train candidates with non-positive calories/mass or negative total macros and train IDs missing from metadata become stable typed `excludedDishes`, and lock/manifest overlap, duplicate exclusions, tampered reasons, omissions, additions, or order drift fail closed before image substitution or writes.
- [x] **Step 4b: Implement and verify exclusion provenance.** Bind the canonical exclusion list into the strict source-lock hash, reconstruct it from pinned source bytes in both pin and verify, preserve all ingredient-level structural validation, and run the focused corpus/schema/public-manifest suite plus build and lint before retrying the one-time pin.
- [x] **Step 4c: Add RED coverage for optional descriptive ingredient names.** Prove an empty ingredient name parses as `''` when its ID/group/numeric fields are valid, while an empty ingredient ID still fails structurally.
