...
| 5 | in progress | OpenCode stalled twice; host fallback active | pre-review green | pending | Step 1 RED tests written at baseline 4454727; RED command pending. User authorized runtime device verification when needed, with explicit physical ADB serial only; never auto-select and never use emulator-5554. |
