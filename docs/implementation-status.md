...
| 4 | in progress | OpenCode unavailable; approved headless fallback unavailable; bounded host fallback for tracking only | pre-review blocked by MCP timeout | pending | Plan corrected for host-only paint isolation; no product code or tests changed yet; no device/emulator |
