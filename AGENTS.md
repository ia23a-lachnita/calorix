...
- Do not use the deprecated Gemini CLI or the `agy` CLI for reviews. Use the Antigravity MCP `ask-ai` tool with `approvalMode: "yolo"`, and a persistent `conversationId` per work stream. Model routing order: (1) `gemini-3.6-flash` primary, (2) `gemini-3.1-pro` fallback, (3) `gemini-3.5-flash` final fallback. The MCP/agy layer supplies High reasoning effort separately; display labels like "(High)" must not be passed in the model parameter. If one route fails before review, try the next in order and record the exact error.
