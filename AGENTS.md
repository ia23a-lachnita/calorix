...
then speed. `muse-spark-1.3-contributor-free` supersedes Muse 1.2;
`mimo-v2.5-free` is excluded by explicit user instruction; `big-pickle` is an
opaque experimental reserve. Do not route to a model absent from the refreshed
`opencode/` namespace.
...
- Do not use the deprecated Gemini CLI or the `agy` CLI for reviews. Use the Antigravity MCP `ask-ai` tool with `approvalMode: "yolo"`, and a persistent `conversationId` per work stream. Model routing order: (1) `gemini-3.8-flash` primary, (2) `gemini-3.7-flash` fallback, (3) `gemini-3.6-flash` final fallback. The MCP/agy layer supplies High reasoning effort separately; display labels like "(High)" must not be passed in the model parameter. `gemini-3.1-pro` is opt-in only for an explicitly justified maximum-reasoning review, not part of routine fallback (if an opt-in `gemini-3.1-pro` call fails, fall back to `gemini-3.8-flash`). `gemini-3.5-flash` is removed from the agy review route because it is SDK/OpenRouter-only. If one route fails before review, try the next in order and record the exact error.
- Every review prompt must explicitly say: **Do not edit files, do not run write commands, and do not mutate the repository; only inspect, reason, debug, review, and propose changes for the main agent to apply.**
