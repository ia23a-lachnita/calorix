...
export class OffProviderError extends Error {
  readonly code = 'provider_request_failed' as const;

  constructor() {
    super('Open Food Facts provider request failed');
    this.name = 'OffProviderError';
  }
}
  ...
  try {
    ...
    try {
      ...
      if (payload.status === 'failure') {
        if (result?.id === 'product_not_found') return null;
        throw new OffProviderError();
      }
      ...
      if (error instanceof OffProviderError) throw error;
      return null;
    }
  } finally {
